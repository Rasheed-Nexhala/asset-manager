...
export const ItemForm: React.FC<ItemFormProps> = ({
  ...
  const weightConfig = useMemo(() => {
    if (formData.type === 'fuel') return null;
    ...
    return null;
  }, [formData.type, selectedSteelMaster, mode, initialData?.weightPerMeter, initialData?.lengthPerPiece]);
  ...
  /** Item type: fuel forces Liters; leaving fuel resets unit for non–weight items */
  const applyTypeChange = useCallback(
    (t: ItemType) => {
      if (typeChangeLocked) return;
      setFormData((prev) => {
        let unit = prev.unit;
        if (t === 'fuel') unit = 'Liters';
        else if (prev.type === 'fuel') unit = 'Pcs';
        return { ...prev, type: t, unit };
      });
      if (t === 'fuel') {
        setSelectedSteelMaster(null);
        setAutoGeneratedSku('');
      }
      setErrors((e) => ({ ...e, type: undefined, unit: undefined }));
    },
    [typeChangeLocked]
  );
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (mode === 'create') {
      ...
      const updateData: UpdateItemData = {
        ...
        categoryId,
        type: formData.type,
        unit: formData.type === 'fuel' ? 'Liters' : formData.unit.trim(),
        minStockLevel: minStockLevelPieces,
  ...
  return (
    ...
    >
      <View className="gap-4 px-4 py-4">
        {/* Item Type — create + edit */}
        {(mode === 'create' || mode === 'edit') && (
          <View className="gap-1.5">
            <Text className="text-[15px] text-[#0F172A]">Item Type <Text className="text-[#DC2626]">*</Text></Text>
            {typeChangeLocked && (
              <View className="mb-1 flex-row gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
                <Ionicons name="warning" size={20} color="#d97706" />
                <Text className="flex-1 text-[13px] text-[#92400e]">
            ...
            <View className="flex-row gap-2">
              <TouchableOpacity
                ...
                onPress={() => applyTypeChange('consumable')}
                activeOpacity={0.7}
              ...
              <TouchableOpacity
                ...
                onPress={() => applyTypeChange('non_consumable')}
                activeOpacity={0.7}
                disabled={loading || typeChangeLocked}
