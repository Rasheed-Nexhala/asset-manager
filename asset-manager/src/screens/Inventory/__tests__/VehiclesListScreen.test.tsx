...
const mockListVehicles = jest.fn().mockResolvedValue([
  ...
  {
    id: 'v2',
    vehicleNumber: 'KA-55-ZZ-9',
    vehicleNumberNormalized: 'ka-55-zz-9',
    createdAt: '2025-01-01T00:00:00.000Z',
    updatedAt: '2025-01-01T00:00:00.000Z',
    createdByUserId: 'u1',
    createdByName: 'Test',
...
describe('VehiclesListScreen', () => {
  ...
  it('filters by search query', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('KA-55-ZZ-9')).toBeTruthy();
    });

    const search = screen.getByLabelText('Search vehicles');
    fireEvent.changeText(search, 'ka-55');

    await waitFor(() => {
      expect(screen.queryByText('MH-12-AB-1')).toBeNull();
    });
    expect(screen.getByText('KA-55-ZZ-9')).toBeTruthy();
  });
});
