...
/**
 * Keeps Redux `activeManagedSiteId` valid for Site Managers and syncs with AsyncStorage.
 */
export function useSiteManagerActiveSiteSync(): void {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const stored = await loadActiveManagedSiteId(userId);
      if (cancelled) return;
      ...
      await saveActiveManagedSiteId(userId, first);
    })();
