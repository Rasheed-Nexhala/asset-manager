vi.mock('../../thunks/activityLogThunks', () => {
...
const mockLog = {
  ...
  actionCategory: 'inventory' as const,
...
describe('activityLogSlice', () => {
  const initialState = {
    logs: [] as import('../../types/activityLog').ActivityLog[],
    totalCount: null as number | null,
    hasMore: true,
    lastDoc: null,
    myRecentActivity: [] as import('../../types/activityLog').ActivityLog[],
    ...
    errorTimestamp: null as number | null,
  };
  ...
  it('setFilters merges filters', () => {
    const state = activityLogReducer(
      initialState,
      setFilters({ actionType: 'item_created' })
    );
    expect(state.filters.actionType).toBe('item_created');
  ...
  it('fetchActivityLogs.fulfilled sets logs', () => {
    ...
    const action = fetchActivityLogs.fulfilled(payload, 'req1', { arg: undefined });
    const state = activityLogReducer(initialState, action);
    expect(state.logs).toEqual([mockLog]);
  ...
  it('fetchMyActivityPaginated.fulfilled sets myRecentActivity and pagination', () => {
    ...
    const action = fetchMyActivityPaginated.fulfilled(payload, 'req1', { arg: undefined });
    const state = activityLogReducer(initialState, action);
