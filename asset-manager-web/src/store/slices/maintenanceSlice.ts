    ...
    builder
      ...
      .addCase(returnFromMaintenanceThunk.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          if (action.meta.arg.returnData.returnQuantity !== undefined) {
            state.maintenanceRecords[index].returnedQuantity = action.meta.arg.returnData.returnQuantity;
          }
        }
        if (state.selectedMaintenance?.id === id) {
          state.selectedMaintenance.status = 'returned';
          if (action.meta.arg.returnData.returnQuantity !== undefined) {
            state.selectedMaintenance.returnedQuantity = action.meta.arg.returnData.returnQuantity;
          }
        }
