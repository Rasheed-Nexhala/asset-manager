...
/** Matches ActivityLogState from activityLogSlice */
const defaultActivityLogState: RootState['activityLog'] = {
  logs: [],
  totalCount: null,
  hasMore: true,
  lastDoc: null,
...
/** Returns ActivityLog-compatible mock with actionCategory, userId, timestamp */
const mockLog = (
  id: string,
  overrides: Partial<{ actionCategory: string; userId: string; timestamp: string }> = {}
) =>
  ({
    id,
...
describe('activityLogSelectors', () => {
  ...
  it('selectActivityLogFilters returns filters', () => {
    ...
    expect(selectActivityLogFilters(state)).toEqual(
