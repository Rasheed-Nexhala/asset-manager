 ...
 * @param payload - Optional. Pass { mode: 'my' } to export current user's requests.
 */
export const exportRequestsThunk = createAsyncThunk(
  'requests/export',
  async (
    payload: { mode?: 'queue' | 'my' } = {},
    ...
    try {
      ...
      const listFilters: RequestListFilters =
        payload?.mode === 'my' && userId
          ? { userId }
          : { status: filters.status, siteId: filters.siteId };

      const requests = await requestService.exportRequestsData(listFilters);
