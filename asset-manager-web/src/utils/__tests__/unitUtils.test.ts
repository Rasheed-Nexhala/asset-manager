...
describe('unitUtils', () => {
  describe('isDiscreteUnit', () => {
    ...
    it('returns false for empty or unknown units', () => {
      ...
      // Web COMMON_UNITS includes Nos / Pair, but they are not "discrete" list items (Pieces/Bags/…)
  ...
  describe('COMMON_UNITS', () => {
    it('includes expected construction/inventory units (web list)', () => {
      expect(COMMON_UNITS).toContain('Nos');
      expect(COMMON_UNITS).toContain('Pcs');
      expect(COMMON_UNITS).toContain('Pair');
  ...
  describe('normalizeUnitForDisplay', () => {
    ...
    it('preserves other units', () => {
      ...
      expect(normalizeUnitForDisplay('Nos')).toBe('Nos');
      expect(normalizeUnitForDisplay('Pair')).toBe('Pair');
    });
  });
});
