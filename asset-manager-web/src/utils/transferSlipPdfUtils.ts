/**
 * Material transfer slip — Web
 * Same pattern as poPdfUtils: company branding + HTML + window.print().
 */
...
/**
 * Deterministic document id for a completed transfer (same request + transfer time → same id).
 */
...
/**
 * Printable HTML for a material transfer slip (proof of dispatch / receipt).
 */
export function generateTransferSlipHtml(
  ...
  const logoImg = logoDataUrl
    ? `<img src="${escapeHtml(logoDataUrl)}" alt="${escapeHtml(companyConfig.logoAlt)}" style="height: 56px; width: auto; display: block;" />`
    : typeof window !== 'undefined'
      ? `<img src="${window.location.origin}${companyConfig.logoPath}" alt="${escapeHtml(companyConfig.logoAlt)}" style="height: 56px; width: auto; display: block;" />`
      : '';
...
<head>
  <meta charset="utf-8">
  <title>Transfer Slip ${escapeHtml(slipId)}</title>
  <style>
...
/**
 * Open print dialog with the transfer slip (or save as PDF from the browser).
 */
export async function printTransferSlip(request: Request): Promise<void> {
  ...
  const html = generateTransferSlipHtml(request, logoDataUrl || undefined);
  const printWindow = window.open('', '_blank');
  if (!printWindow) {
    throw new Error('Popup blocked. Allow popups to print or save the transfer slip.');
  }
