...
import { adjustQuantity, getItemById } from './inventoryService';
 ...
 * Uses inventory service directly (not adjustQuantity thunk) to avoid duplicate quantity_adjusted logs
 * and to skip Store Incharge central-store access-window check for this dedicated fuel flow.
 */
export async function assignFuelToVehicle(
  ...
  const { oldQuantity, newQuantity } = await adjustQuantity({
    itemId: data.itemId,
    itemName: item.name,
    itemSku: item.sku,
    locationId,
  ...
  try {
    ...
    try {
      await adjustQuantity({
        ...
        type: 'add',
        quantity: qty,
        reason: 'Rollback after assignment record failed',
        notes: 'System',
      });
    } catch {
      // ignore
