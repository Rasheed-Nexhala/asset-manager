...
export function AllocateFuelToVehiclesPage() {
  ...
  const [allocationSuccess, setAllocationSuccess] = useState<SuccessSummary | null>(null);
  ...
  const handleSubmit = useCallback(async () => {
    if (!siteId || !userId || !selectedRequest || !selectedLine || !selectedVehicle) {
      toast.info('Choose request, fuel line, vehicle, and quantity (liters).');
      return;
    }
    const q = parseFloat(qty);
    if (!Number.isFinite(q) || q <= 0) {
      toast.info('Enter a valid quantity in liters.');
      return;
    }
    if (q > availableForLine) {
      toast.info(`Only ${availableForLine} L can still be dispensed on this line.`);
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast.success(`Dispensed ${q} L to ${selectedVehicle.vehicleNumber}.`);
      setQty('');
      setSelectedVehicleId(null);
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : 'Could not dispense fuel');
    } finally {
      setSubmitting(false);
    }
  }, [
    ...
    return (
      <div className="flex h-full flex-col">
        <header className="flex items-center gap-4 pb-4">
          ...
          <h1 className="text-[22px] font-semibold text-[#0F172A]">Dispense fuel</h1>
        </header>
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <header className="flex shrink-0 items-center gap-4 pb-2">
        ...
        <div>
          <h1 className="text-[22px] font-semibold text-[#0F172A]">Dispense fuel</h1>
          <p className="text-[13px] text-[#64748B]">From transferred requests to vehicles</p>
        </div>
      </header>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-1 pb-8">
          <div className="mb-4 flex gap-2 rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] p-3">
            ...
            <p className="text-[13px] leading-5 text-[#64748B]">
              Fuel lines only. Liters are recorded on the vehicle and deducted from the quantity still available on
              this request. This does not return stock to the central store.
            </p>
          </div>
              ...
              <div className="mb-2 flex items-start justify-between gap-2">
                <div className="flex min-w-0 flex-1 items-start gap-2">
                  ...
                  <div className="min-w-0">
                    <p className="text-[15px] font-semibold text-[#0F172A]">Fuel dispensed</p>
                    ...
                    <p className="mt-1 text-[15px] text-[#0F172A]">
                      <span className="font-semibold">{allocationSuccess.quantity}</span> L →{' '}
                      <span className="font-semibold">{allocationSuccess.vehicleNumber}</span>
                    </p>
                  </div>
                </div>
                <button
                      ...
                      <li key={line.itemId}>
                        ...
                        >
                          ...
                          <div className="flex gap-8 text-[13px] text-[#64748B]">
                            <span>
                              At site:{' '}
                              <span className="font-medium text-[#0F172A]">{maxAtSite} L</span>
                            </span>
                            <span>
                              Dispensed:{' '}
                              <span className="font-medium text-[#0F172A]">{dispensed} L</span>
                            </span>
                          </div>
                        </button>
                      </li>
          ...
          {vehicles.length === 0 ? (
            <div className="mb-8 rounded-[10px] border border-[#E2E8F0] bg-white p-5">
              <p className="mb-1 text-center text-[15px] font-medium text-[#0F172A]">Add vehicles first</p>
              <p className="mb-4 text-center text-[13px] text-[#64748B]">
                Register at least one vehicle in the fleet list.
              </p>
              <Link
                to="/inventory/vehicles/new"
                className="flex min-h-[48px] items-center justify-center gap-2 rounded-[10px] border-2 border-[#1E40AF] text-[15px] font-semibold text-[#1E40AF] hover:bg-[#EFF6FF]"
              >
            ...
            <ul className="mb-8 space-y-2">
              {vehicles.map((v) => (
                <li key={v.id}>
                  ...
                  >
                    <span className="flex-1 text-[15px] font-semibold text-[#0F172A]">{v.vehicleNumber}</span>
                    {selectedVehicleId === v.id && (
          ...
          <StepBadge n={4} label="Quantity (liters)" />
          <label className="mb-1 block text-[15px] text-[#0F172A]">Liters for this vehicle</label>
          <input
            type="number"
            min={0}
            step="any"
          ...
          {selectedLine && (
            <p className="mb-6 text-[13px] text-[#64748B]">
              You can dispense up to{' '}
              <span className="font-semibold text-[#0F172A]">{availableForLine}</span> more L on this line.
            </p>
          )}
          ...
          <div className="mt-6 rounded-[10px] border border-[#E2E8F0] bg-white p-4">
            <p className="mb-3 text-[13px] font-medium text-[#64748B]">Done?</p>
